...
while [ $login_state == 1 ]
do
	login=$(yad  --username --password --title "登录到作业系统")
	if [ $? != 0 ]
	then
		exit 0
	fi
	...
	if [ $login_state == 1 ]
	then
		yad --error --width 300 --text "登录失败，请检查用户名和密码"
	fi
done
...
case $user_type in
	...
	;;
	3)
		yad --error --width 300 --text "账户类型错误"
		exit 1 
	;;
esac
...
do
	case $on_state in
		0)
			option=$(yad --height=300 --width=280 --title="管理员" --list --text "请选择要进行的操作" --radiolist  --column "选择" \
		...
		1)
			...
			teacher_selected=$(yad --list --title="教师账号列表" --ok-label="修改" --cancel-label="返回" --extra-button --extra-label="绑定课程" --height=600 --width=400 --column="工号" --column="姓名" $teacher_content)
			echo $?
			case $? in
				0) 
					on_state=0
		...
		;;
		2)
			teacher_info=$(yad --forms --title="添加教师"\
			--text="请输入教师账号信息"\
			--add-entry="工号"\
			--add-entry="姓名"\
			...
			if [ "$teacher_id" == "" ]; then
				yad --info --width=250 --text "工号不能为空" 2> >(grep -v GtkDialog >&2)
				continue
			elif [ "$teacher_name" == "" ]; then
				yad --info --width=250 --text "姓名不能为空" 2> >(grep -v GtkDialog >&2)
				continue
			elif [ "$teacher_passwd" == "" ]; then
				yad --info --width=250 --text "密码不能为空" 2> >(grep -v GtkDialog >&2)
				continue
			fi
...
EOF
			case $? in
				0) 
					yad --info --width=150 --text "插入成功" 2> >(grep -v GtkDialog >&2)
					on_state=0
				;;
				1) yad --info --width=150 --text "插入失败" 2> >(grep -v GtkDialog >&2);;
			esac
		;;
		3)
			teacher_id=$(yad --forms --title="删除教师"\
			--text="请输入教师工号"\
...
EOF
			...
			case $? in
				0) 
					yad --info --width=150 --text "删除成功" 2> >(grep -v GtkDialog >&2)
					on_state=0
				;;
				1) yad --info --width=150 --text "删除失败" 2> >(grep -v GtkDialog >&2);;
			esac
		;;
		4)
			teacher_info=$(yad --forms --width=300 --title="修改教师信息" --text="工号：$temp_teacher_id"\
			--add-entry="姓名"\
			--add-entry="密码" 2> >(grep -v GtkDialog >&2))
			if [ $? == 1 ]; then
				on_state=1
...
EOF
			case $? in
				0) 
					yad --info --width=150 --text "修改成功" 2> >(grep -v GtkDialog >&2)
					on_state=0
				;;
				1) yad --info --width=150 --text "修改失败" 2> >(grep -v GtkDialog >&2);;
			esac
		;;
		5)
			course_content=$($MYSQL -u$username -p$password homework -Bse "select * from course;")
			course_selected=$(yad --list --title="课程列表" --height=600 --width=400 --column="课号" --column="课程名" $course_content)
			case $? in
				0) 
					on_state=0
		...
		;;
		6)
			course_info=$(yad --forms --title="添加课程"\
			--text="请输入课程信息"\
			--add-entry="课号"\
			...
			if [ "$course_id" == "" ]; then
				yad --info --width=250 --text "课号不能为空" 2> >(grep -v GtkDialog >&2)
				continue
			elif [ "$course_name" == "" ]; then
				yad --info --width=250 --text "课程名不能为空" 2> >(grep -v GtkDialog >&2)
				continue
			fi
			$MYSQL -u$username -p$password homework <<EOF
...
EOF
		...
		;;
		7)
			course_id=$(yad --forms --title="删除课程"\
			--text="请输入课程课号"\
...
EOF
		...
		;;
		8)
			course_name=$(yad --forms --width=300 --title="修改课程信息" --text="课号：$temp_course_id"\
			--add-entry="课程名" 2> >(grep -v GtkDialog >&2))
			if [ $? == 1 ]; then
				on_state=5
				continue
...
EOF
		...
		;;
		10)
			option=$(yad --height=300 --width=280 --title="教师" --list --text "请选择要进行的操作" --radiolist  --column "选择" \
